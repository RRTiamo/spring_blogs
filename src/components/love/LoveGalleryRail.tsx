...
import { MapPin, Heart, ChevronLeft, ChevronRight, Sparkles } from "lucide-react";
import type { LoveEntry } from "@/interface/love";
import ParallaxImage from "@/components/ui/ParallaxImage";
import gsap from "gsap";

interface LoveGalleryRailProps {
  photos: LoveEntry[];
  onSelect: (photo: LoveEntry) => void;
}
...
export default function LoveGalleryRail({ photos, onSelect }: LoveGalleryRailProps) {
  ...
  const activeSpeed = useRef(0.6); // 比时间轴稍微慢一点，更适合阅读
  ...
  // 复制 photos 保证无缝 Marquee 循环长度
  const displayPhotos = useMemo(() => {
    if (photos.length === 0) return [];
    const minItems = 12;
    const repeatCount = Math.max(3, Math.ceil(minItems / photos.length));
  ...
  // 自动播放帧循环 (仅在视口内时按需启用)
  useEffect(() => {
    if (!isDesktop || photos.length === 0 || !isInView) {
      if (rafRef.current) cancelAnimationFrame(rafRef.current);
      if (manualTweenRef.current) manualTweenRef.current.kill();
      if (trackRef.current && (!isDesktop || photos.length === 0)) {
    ...
    const singleCycleWidth = photos.length * step;
            ...
            >
              ...
              <div className="space-y-3 px-1 pb-2 pt-4">
                ...
                <div className="flex items-center justify-between">
                  <p className="flex items-center gap-1.5 truncate text-[12px] text-charcoal/45 dark:text-cream/45">
                    <Heart className="h-3.5 w-3.5 text-rose-500 fill-rose-500/20" />
                    <span>心情: {moodLabelMap[entry.mood] || entry.mood}</span>
                  </p>
